...
from .setup import *
from .cooltv_handle import Cooltv
...
class ModuleMain(PluginModuleBase):
    ...
    def process_command(self, command, arg1, arg2, arg3, req):
        if command == 'broad_list':
            ret = {'ret':'success', 'ch_list':Cooltv.ch_list()}
    ...
    def process_api(self, sub, req):
        try:
            if sub == 'm3u':
                return Cooltv.make_m3u()
            elif sub == 'url.m3u8':
                mode, data = Cooltv.get_m3u8(req.args.get('ch_id'))
                if mode == 'text':
                    return data
                else:
                    return redirect(data)
            elif sub == 'segment.ts':
                return Cooltv.segment(req)
